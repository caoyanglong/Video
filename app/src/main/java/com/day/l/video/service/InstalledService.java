...
public class InstalledService extends Service {
    ...
    private void installFile(long id) {
        ...
        Uri downloadFileUri = ((DownloadManager)getSystemService(DOWNLOAD_SERVICE)).getUriForDownloadedFile(id);
        Log.d("<downloaduri>",downloadFileUri.toString());
        Cursor cursor = getContentResolver().query(downloadFileUri,null,null,null,null);

        while (cursor.moveToNext()){
            Log.d("cursor_data",cursor.toString());
        }
